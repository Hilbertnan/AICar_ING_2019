...
int servo_data = 115;//舵机接受数据
int ANG_L = 115;//小车左转角度
int ANG_mid = 80;//小车正向角度
int ANG_R = 45;//小车右转角度
int angle = 115;//定义转向角度存储器，初始正向
...
void input_data() {
  ...
  if (mark == 1) //如果接收到数据则执行comdata分析操作，否则什么都不做。
  {
    ...
    comdata = String("");
    if(motor_data_temp >= 1000 && motor_data_temp <= 1400){
...
void Servo_logic(){
  //舵机逻辑部分
  if (servo_data > angle) {ANG_L = servo_data;Serial.println(angle);}
